...
4. `Extract` runs only after `Match` succeeds. Return a non-nil `Root` on success. Return source-specific metadata, ordered samples, the subtree that the rendered samples replace (`SamplesAnchor`, also listed in `Excluded`), exact subtrees that are safe to exclude, and warnings for non-fatal observations. A nil `SamplesAnchor` appends the rendered samples after `Root`.
