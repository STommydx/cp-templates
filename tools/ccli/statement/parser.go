...
func parseWithAdapter(capture *CaptureEnvelope, document *nethtml.Node, adapter Adapter) (ParseResult, error) {
	...
	markdown := RenderMarkdown(metadata.Identity.Title, capture.URL, capture.CapturedAt, extraction.Root, extraction.Excluded, extraction.Samples, false)
...
func fallbackResult(capture *CaptureEnvelope, document *nethtml.Node, reason string) (ParseResult, error) {
	...
	return ParseResult{
		Markdown: RenderMarkdown(capture.Title, capture.URL, capture.CapturedAt, root, nil, nil, true),
		Mode:     "fallback",
		Metadata: metadata,
